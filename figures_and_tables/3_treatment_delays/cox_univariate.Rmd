...
# Univariate Results
```{r echo=TRUE, message=FALSE, warning=FALSE}
run_univariate <- function(data, additional_predictors=c(), dependent="regular") {
  ...
  if (dependent == "regular") {
    dependent <- "Surv(time_alive_treat_mo, vital_status)"
    
    data %>%
      finalfit(dependent, uni_predictors) %>%
      select("Dependent: Surv(time_alive_treat_mo, vital_status)", " ", 
             "all", "HR (univariable)") %>%
      knitr::kable()
  }
  else {
    dependent <- "Surv(time_alive_treat_mo24, vital_status24)"
    
    data %>%
      finalfit(dependent, uni_predictors) %>%
      select("Dependent: Surv(time_alive_treat_mo24, vital_status24)", " ", 
             "all", "HR (univariable)") %>%
      knitr::kable()
  }
