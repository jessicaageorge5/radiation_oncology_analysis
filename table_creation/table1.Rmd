...
label(all_patients$enroll_age) <- "Age"
label(all_patients$age_cat) <- ""
units(all_patients$enroll_age) <- "y"
label(all_patients$marital) <- "Marital status"
label(all_patients$cancer_screening) <- "Previously screened for cervical cancer"
label(all_patients$distance) <- "Distance from treatment facility"
units(all_patients$distance) <- "km"
label(all_patients$combined_cancer_stage) <- "Disease stage"
label(all_patients$vaginal_hemorrhage_symp) = "Vaginal bleeding"
label(all_patients$vaginal_discharge_symp) = "Vaginal discharge"
label(all_patients$pelvic_pain_symp) = "Pelvic/back pain"
label(all_patients$cr_result) <- "Creatinine"
units(all_patients$cr_result) <- "&mu;mol/L"
label(all_patients$hb_result) <- "Hemoglobin"
units(all_patients$hb_result) <- "g/dL"
label(all_patients$neut_result) <- "ANC"
units(all_patients$neut_result) <- "&times;10<sup>9</sup>/L"
label(all_patients$wbc_result) <- "WBC"
units(all_patients$wbc_result) <- "&times;10<sup>9</sup>/L"
label(all_patients$init_performance_status) <- "Baseline performance status"
units(all_patients$init_performance_status) <- "KPS"
label(all_patients$cd4_final) <- "CD4"
units(all_patients$cd4_final) <- "cells/&mu;L"
label(all_patients$cd4_pos_cat) = "CD4 category"
label(all_patients$on_arv) <- "On ARV"
label(all_patients$total_chemo_received) <- "No. of chemo cycles received"
label(all_patients$chemo) <- "Chemo/no chemo"
label(all_patients$ebrt_40) <- "Received EBRT dose >= 40 Gy"
label(all_patients$brachy_20) <- "Received brachytherapy dose >= 20 Gy"
label(all_patients$eqd2_75) <- "Received EQD2 dose >= 75 Gy"
label(all_patients$eqd2) <- "EQD2"
units(all_patients$eqd2) <- "Gy"
label(all_patients$treat_duration) <- "Treatment duration"
units(all_patients$treat_duration) <- "d"
label(all_patients$treat_response) <- "Tumor response"
```
...
make.table(dat = all_patients,
           ...
           cat.varlist = c("age_cat", "marital", "cancer_screening", "combined_cancer_stage", "vaginal_hemorrhage_symp", "vaginal_discharge_symp", "pelvic_pain_symp", "init_performance_status", "chemo", "ebrt_40", "brachy_20", "eqd2_75", "treat_response", "surgery"),
...
# cd4_pos_cat2
# counts
data %>% filter(hiv_status == "HIV-infected") %>% group_by(cd4_pos_cat2) %>% summarise(n())
...
# vl_detect
data %>% filter(hiv_status == "HIV-infected") %>% group_by(vl_detect) %>% summarise(n())
```