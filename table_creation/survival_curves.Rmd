...
1) Survival by HIV status
```{r}
hiv_survival <- survfit(Surv(time_alive_years, vital_status) ~ hiv_status, data)
...
2) Survival by chemoradiation therapy
```{r}
chemo_rad_survival <- survfit(Surv(time_alive_years, vital_status) ~ chemo_rad, data)
...
```{r}
hiv_chemo_rad_survival <- survfit(Surv(time_alive_years, vital_status) ~ hiv_status + chemo_rad, data)
